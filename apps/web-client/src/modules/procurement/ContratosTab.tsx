...
export const ContratosTab: React.FC<{ contratos: Contrato[] }> = ({ contratos }) => {
    return (
        <div className="bg-white border border-border rounded-xl p-6">
            <h3 className="text-lg font-bold text-foreground mb-4 font-mono">Contratos Administrativos</h3>
                ...
                <tbody className="font-mono tabular-nums">
                    {contratos.map(c => (
                        <tr key={c.id} className="border-b border-border">
                            ...
                            <td className="py-2"><span className="px-2 py-0.5 rounded bg-success/10 text-success">{c.status}</span></td>
                        </tr>
                    ))}
                </tbody>
            </table>
