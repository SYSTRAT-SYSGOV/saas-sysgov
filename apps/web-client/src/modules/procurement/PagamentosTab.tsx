...
export const PagamentosTab: React.FC<{ pagamentos: Pagamento[] }> = ({ pagamentos }) => {
    ...
    return (
        <div className="bg-white border border-border rounded-xl p-6">
            <h3 className="text-lg font-bold text-foreground mb-4 font-mono">Liquidação & Pagamentos</h3>
                    ...
                    {pagamentos.map(p => {
                        ...
                        return (
                            <tr key={p.id} className="border-b border-border">
                                ...
                                <td className="py-2">
                                    {prazoExcedido ? (
                                        <span className="px-2 py-0.5 rounded bg-destructive/10 text-destructive font-bold">RN-008: +{LIMITE_DIAS}d</span>
                                    ) : (
                                        <span className="px-2 py-0.5 rounded bg-success/10 text-success">{p.status}</span>
                                    )}
                                </td>
                            </tr>
                        );
