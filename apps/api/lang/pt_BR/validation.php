...
// Mensagens padrão de validação do Laravel traduzidas para pt_BR.
// Sem este arquivo, o app.locale=pt_BR não tinha efeito nas mensagens
// de validação (só existe o pacote en/validation.php no vendor) — todo
// erro de formulário (obrigatoriedade, tipo inválido, tamanho etc.) saía
// em inglês para o usuário final, em qualquer módulo do sistema, não só
// no Licita. `attributes` abaixo troca o nome técnico do campo (ex.:
// "campos.0.tipo") por algo legível quando o backend não usa
// ->validate([...], [], [attributes customizados no Request]).
...
return [
    ...
    /*
    |--------------------------------------------------------------------------
    | Nomes amigáveis de atributos (padrão do sistema)
    |--------------------------------------------------------------------------
    |
    | Convertem o path técnico do campo (o que o front manda, ex.:
    | "campos.0.tipo", "texto_completo") em algo legível na mensagem final
    | ("O campo Tipo do Campo é obrigatório."), sem cada módulo precisar
    | passar $messages/$customAttributes manualmente no ->validate(). Os
    | índices de array (":attribute" com "campos.0.*") o Laravel já
    | resolve sozinho contra a entrada mais específica cadastrada aqui.
    |
    */

    'attributes' => [
        'campos' => 'campos',
        'campos.*.key' => 'chave do campo',
        'campos.*.label' => 'rótulo do campo',
        'campos.*.tipo' => 'tipo do campo',
        'campos.*.opcoes' => 'opções do campo',
        'campos.*.obrigatorio' => 'obrigatoriedade do campo',
        'campos.*.ordem' => 'ordem do campo',
        'campos.*.ajuda' => 'texto de ajuda do campo',
        'campos.*.aba' => 'aba do campo',
        'tipo' => 'tipo',
        'numero' => 'número',
        'titulo' => 'título',
        'ementa' => 'ementa',
        ...
        'itens.*.unidade_medida' => 'unidade de medida do item',
