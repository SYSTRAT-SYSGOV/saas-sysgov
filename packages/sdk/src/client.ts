...
export type ApiModule = { id: number; name: string; alias: string; enabled: boolean; metadata?: Record<string, unknown>; description?: string; icon?: string; menu_label?: string; menu_order?: number; permissions?: string[]; menu_items?: Array<{ id: number; label: string; route: string; icon: string; permission?: string; order: number }> };
...
import type { ApiRequester, RequestOptions } from './modules/base';
import { OrgModuleClient } from './modules/org';
import { ContractsModuleClient } from './modules/contracts';
...
export class SysgovApi implements ApiRequester {
  ...
  public readonly org: OrgModuleClient;
  public readonly contractsApi: ContractsModuleClient;
  ...
  constructor(private readonly baseUrl = 'http://localhost:8000/api') {
    ...
    this.contractsApi = new ContractsModuleClient(this);
